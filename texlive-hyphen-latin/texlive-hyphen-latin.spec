...
Source0: ftp://ftp.ctex.org/mirrors/texlive/tlpretest/archive/hyphen-latin.tar.xz

Name: texlive-hyphen-latin
License: LPPL
Summary: Hyphenation patterns for the Latin language
Version: %{tl_version}
Release: %{tl_noarch_release}.3.1.svn18673%{?dist}
BuildArch: noarch
...
echo "latin loadhyph-la.tex" >> %{_texdir}/texmf/tex/generic/config/language.dat
echo "\addlanguage{latin}{loadhyph-la.tex}{}{2}{2}" >> %{_texdir}/texmf/tex/generic/config/language.def
touch /var/run/texlive/run-fmtutil
:

%postun
if [ $1 == 0 ]; then
  sed -i '/latin loadhyph-la.tex/d' %{_texdir}/texmf/tex/generic/config/language.dat
  sed -i '/\\addlanguage{latin}{loadhyph-la.tex}{}{2}{2}/d' %{_texdir}/texmf/tex/generic/config/language.def
